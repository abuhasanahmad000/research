...
// ─────────────────────────────────────────────────────────────
async function searchOpenAlex(keywords, opts = {}) {
  const {
    yearFrom = 2019,
    yearTo = 2025,
    types = [],          // OpenAlex type IDs e.g. ["article","review"]
    openAccessOnly = false,
    scopusOnly = false,  // post-filter using ISSN lookup
    qFilter = [],        // post-filter using ISSN lookup
  } = opts;

  try {
    // Match OpenAlex website: uses `search` parameter for OR query
    const q = keywords.join(" OR ");

    // Build filter array (matches OpenAlex web filter syntax)
    const filters = ["publication_year:" + yearFrom + "-" + yearTo];
    if (types.length > 0) {
      filters.push("type:" + types.join("|"));  // OpenAlex pipe = OR
    }
    if (openAccessOnly) {
    ...
    const allResults = [];
    let cursor = "*";
    const MAX_PAGES = 25; // safety cap = 5000 raw articles (more needed since Scopus filter trims results)
    let metaCount = 0;
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      const url = "https://api.openalex.org/works"
        ...
        + "&select=id,title,authorships,publication_year,primary_location,host_venue,abstract_inverted_index,doi,concepts,cited_by_count,type,open_access"
      ...
      if (!nextCursor) break;
      cursor = nextCursor;
      // Stop early if we have enough — but keep fetching if Scopus filter is on
    ...
    const mapped = allResults.map(w => {
      ...
      const primarySource = w.primary_location?.source || w.host_venue || {};
      const journal = primarySource.display_name || w.host_venue?.display_name || "Unknown Journal";

      // Get all ISSNs from the source for lookup
      const issns = [];
      if (primarySource.issn_l) issns.push(primarySource.issn_l);
      if (Array.isArray(primarySource.issn)) issns.push(...primarySource.issn);
      if (w.host_venue?.issn_l) issns.push(w.host_venue.issn_l);
    ...
    // POST-FILTERS: applied after fetch since OpenAlex doesn't have these filters natively
    let filtered = mapped;

    // Scopus filter (ISSN-based)
    if (scopusOnly) {
      filtered = filtered.filter(a => a.inScopus);
    }

    // Q filter (only applied if user selected Q values)
    if (qFilter.length > 0) {
      filtered = filtered.filter(a => qFilter.includes(a.q));
    }

    return { articles: filtered, totalAvailable: metaCount, fetchedRaw: allResults.length };
  } catch (e) {
...
["keyword1","keyword2","keyword3"]`,
  ...
  async function runSearch() {
    ...
    try {
      const wantScopus = (searchParams.databases || []).includes("scopus");

      const result = await searchOpenAlex(keywords, {
        yearFrom: searchParams.yearFrom,
        yearTo: searchParams.yearTo,
        types: searchParams.types || [],
        openAccessOnly: searchParams.openAccessOnly || false,
        scopusOnly: wantScopus,
        qFilter: searchParams.qIndex || [],
      });
...
// ═══════════════════════════════════════════════
function TabSearch({ theme, setTheme, keywords, setKeywords, kwInput, setKwInput, searchParams, setSearchParams, rawArticles, searchDone, searchTotalCount, aiStatus, suggestKeywords, runSearch, downloadCSV, setTab }) {
      ...
      <div className="card">
        ...
        <div className="fg" style={{ marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 7, lineHeight: 1.5 }}>
            Filter berbasis ISSN cross-check terhadap Scopus Sources List resmi (54.011 ISSN, Mar 2026). Artikel yang jurnalnya tidak terindeks Scopus akan disaring keluar.
          </div>
          <div className="chip-group">
            {[
              { id: "scopus", label: "Scopus", desc: "Elsevier · 31.570 sumber aktif" },
            ].map(db => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontSize: 9, color: isOn ? "rgba(79,156,249,.7)" : "var(--muted)", fontStyle:"italic" }}>{db.desc}</span>
                </span>
              );
            })}
          </div>
        ...
        <div className="fg" style={{ marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 7, lineHeight: 1.5 }}>
            Q Index dari ScimagoJR (data lookup ISSN). Artikel yang jurnalnya tidak ada di database Scimago ditandai "Unranked".
          </div>
          <div className="chip-group">
            {Q_OPTS.map(q => (
        ...
        {/* Type Filter */}
        <div className="fg" style={{ marginBottom: 12 }}>
          <label>Tipe Publikasi (centang yang dicari)</label>
        ...
        <div className="fg">
          ...
          <div className="chip-group">
            <span
              ...
              onClick={() => setSearchParams(p => ({ ...p, openAccessOnly: true }))}
            ...
            <span
              className={`chip ${!searchParams.openAccessOnly ? "on" : ""}`}
              onClick={() => setSearchParams(p => ({ ...p, openAccessOnly: false }))}
              style={{ padding: "6px 14px" }}
            >
        ...
        <div className="result-counter">
          <div>
            ...
            <div className="rc-lbl">
              Artikel Sesuai Filter
              {searchTotalCount > 0 && <span style={{ display:"block", fontSize:10, marginTop:3 }}>(dari ~{searchTotalCount.toLocaleString()} total di OpenAlex)</span>}
            </div>
          </div>
          ...
          <div style={{ flex: 1 }}>
            {searchParams.databases.length > 0 && (
              <div style={{ fontSize: 11, color: "var(--muted)" }}>
                Indeks: {searchParams.databases.includes("scopus") && "Scopus"}
              </div>
            )}
            {(searchParams.types || []).length > 0 && (
              <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 3 }}>
                Tipe: {searchParams.types.length} dipilih
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
