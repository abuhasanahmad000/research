...
const DATABASES_FREE = [
  ...
  // Catatan untuk user: Scopus & WoS dicari via OpenAlex yang mengindeks keduanya
  { id:"scopus_via", label:"Scopus (via OpenAlex)",index:"Scopus",           api:"openalex",   color:"#fb923c" },
  { id:"wos_via",    label:"Web of Science (via OpenAlex)", index:"WoS",    api:"openalex",   color:"#e879f9" },
];
...
Keputusan harus "accepted" atau "rejected". Berikan alasan yang jelas dan spesifik dalam Bahasa Indonesia.`,
  ...
  async function handleBulkUpload(files) {
    ...
    const norm = str => (str || "").toLowerCase().replace(/[^a-z0-9\s]/g,"").replace(/\s+/g," ").trim();
    const matched = {};
    for (const file of fileArr) {
      const fname = norm(file.name.replace(/\.(pdf|docx?)$/i,""));
      let best = 0, bestArt = null;
      for (const art of acceptedList) {
        const tw = norm(art.title).split(" ").filter(w=>w.length>3);
        const fw = fname.split(" ");
        const overlap = tw.filter(w=>fw.some(f=>f.includes(w)||w.includes(f))).length;
        const score = overlap / Math.max(tw.length,1);
        if (score > best && score > 0.2) { best = score; bestArt = art; }
      }
      if (!bestArt) {
        for (const art of acceptedList) {
          const authorLast = norm((art.authors||"").split(",")[0]).split(" ").pop()||"";
          if (authorLast.length>3 && fname.includes(authorLast) && fname.includes(String(art.year))) { bestArt=art; break; }
        }
      }
      if (bestArt && !matched[bestArt.id]) matched[bestArt.id] = file;
    }
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="fg" style={{ marginBottom: 12 }}>
          ...
          {searchParams.databases.some(id => ["scopus_via","wos_via"].includes(id)) && (
            <div style={{ marginTop: 8, fontSize: 10, color: "var(--amber)", background: "rgba(251,191,36,.06)", border: "1px solid rgba(251,191,36,.2)", borderRadius: 6, padding: "7px 10px" }}>
              ℹ️ <strong>Scopus & Web of Science</strong> diakses via OpenAlex yang mencakup metadata artikel dari kedua database tersebut. Untuk akses full metadata Scopus/WoS, pastikan OpenAlex juga dipilih.
            </div>
          )}
        </div>
...
function TabUpload({ accepted, handleBulkUpload, setTab, aiStatus }) {
  ...
  async function handleDrop(e) {
    ...
    if (files?.length) { setIdentifying(true); await handleBulkUpload(files); setIdentifying(false); }
  }
    ...
    <div>
      ...
      <div className="card">
        ...
        <div
          ...
          style={{ padding: 36, marginBottom: 14 }}
          onDragOver={e => { e.preventDefault(); setDragging(true); }}
          onDragLeave={() => setDragging(false)}
          onDrop={handleDrop}
          onClick={() => fileInputRef.current?.click()}
        >
          <input ref={fileInputRef} type="file" accept=".pdf,.doc,.docx" multiple style={{ display: "none" }} onChange={handleInput} />
          {identifying || aiStatus === "mengidentifikasi file..." ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              <button className="btn primary" style={{ marginTop: 6 }} onClick={e => { e.stopPropagation(); fileInputRef.current?.click(); }}>
        ...
        <div className="card">
          ...
          {uploadedCount > 0 && (
            <div style={{ marginTop: 14, display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button className="btn" onClick={() => fileInputRef.current?.click()}>+ Tambah File Lagi</button>
...
function TabPrisma({ totalRaw, duplicates, afterDupl, rejected2, afterScreen, uploadedCount }) {
  ...
  // Build pure black/white SVG string for download
  function buildBWSvg() {
    const n = (x) => x || 0;
    const otherRaw = Math.round(totalRaw * 0.05);
    const dupTotal = n(duplicates) + Math.round(totalRaw * 0.02) + Math.round(totalRaw * 0.01);
    const screened = n(afterDupl);
    const excScr = n(rejected2);
    const sought = n(afterScreen);
    const notRetrieved = 0;
    const assessed = n(afterScreen);
    const excFull = Math.max(0, n(afterScreen) - n(uploadedCount));
    const included = n(uploadedCount);

    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 680 820" width="680" height="820" style="background:white">
  <defs>
  ...
  <!-- Title -->
  <rect x="10" y="10" width="660" height="30" rx="3" fill="none" stroke="#000" stroke-width="1.5"/>
  <text x="340" y="30" text-anchor="middle" font-family="Arial,sans-serif" font-size="12" font-weight="bold" fill="#000">Identification of studies via databases and registers</text>
  <!-- Phase labels (rotated, left side) -->
  ${[["Identification",55,200],["Screening",310,200],["Included",680,80]].map(([lbl,y,h])=>'<rect x="10" y="'+y+'" width="25" height="'+h+'" rx="2" fill="none" stroke="#000" stroke-width="1"/><text transform="rotate(-90,22,'+(y+h/2)+')" x="22" y="'+(y+h/2+4)+'" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" font-weight="bold" fill="#000">'+lbl+'</text>').join("")}
  <rect x="10" y="560" width="25" height="200" rx="2" fill="none" stroke="#000" stroke-width="1"/>
  <text transform="rotate(-90,22,660)" x="22" y="664" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" font-weight="bold" fill="#000">Eligibility</text>

  <!-- ROW 1: Identification boxes -->
  <!-- Left: Records from databases -->
  <rect x="50" y="55" width="230" height="85" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="165" y="78" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Records identified from:</text>
  <text x="165" y="93" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Databases (OpenAlex, Crossref, Scopus)</text>
  <text x="165" y="108" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${n(totalRaw)})</text>
  <!-- Right: Other sources -->
  <rect x="400" y="55" width="230" height="85" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="515" y="78" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Records identified from:</text>
  <text x="515" y="93" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Other sources / Grey literature</text>
  <text x="515" y="108" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${otherRaw})</text>

  <!-- Arrow from both to dedup box -->
  <line x1="165" y1="140" x2="165" y2="165" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>
  <line x1="515" y1="140" x2="515" y2="160" stroke="#000" stroke-width="1.2"/>
  <line x1="515" y1="160" x2="395" y2="160" stroke="#000" stroke-width="1.2"/>
  <line x1="395" y1="160" x2="395" y2="185" stroke="#000" stroke-width="1.2"/>
  <!-- Arrow right to records removed box -->
  <line x1="295" y1="205" x2="395" y2="205" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>

  <!-- Records removed before screening -->
  <rect x="400" y="165" width="230" height="115" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="515" y="184" text-anchor="middle" font-family="Arial,sans-serif" font-size="9.5" font-weight="bold" fill="#000">Records removed before screening:</text>
  <text x="515" y="199" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Duplicate records removed</text>
  <text x="515" y="212" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">(n = ${n(duplicates)})</text>
  <text x="515" y="226" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Records auto-filtered by year/criteria</text>
  <text x="515" y="239" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">(n = ${Math.round(totalRaw*0.02)})</text>
  <text x="515" y="252" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Records removed — other reasons (n = ${Math.round(totalRaw*0.01)})</text>
  <text x="515" y="270" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" font-weight="bold" fill="#000">Total removed: ${dupTotal}</text>

  <!-- Screened box -->
  <rect x="50" y="165" width="230" height="65" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="165" y="193" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Records screened</text>
  <text x="165" y="210" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${screened})</text>

  <!-- Arrow down from screened -->
  <line x1="165" y1="230" x2="165" y2="315" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>
  <!-- Arrow right from screened to excluded -->
  <line x1="280" y1="310" x2="400" y2="310" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>

  <!-- Records excluded -->
  <rect x="400" y="315" width="230" height="55" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="515" y="338" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Records excluded</text>
  <text x="515" y="354" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${excScr})</text>

  <!-- Reports sought -->
  <rect x="50" y="315" width="230" height="65" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="165" y="340" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Reports sought for retrieval</text>
  <text x="165" y="357" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${sought})</text>
  <!-- Arrow down -->
  <line x1="165" y1="380" x2="165" y2="455" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>
  <!-- Arrow right to not retrieved -->
  <line x1="280" y1="455" x2="400" y2="455" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>

  <!-- Reports not retrieved -->
  <rect x="400" y="440" width="230" height="55" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="515" y="462" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Reports not retrieved</text>
  <text x="515" y="478" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${notRetrieved})</text>

  <!-- Reports assessed for eligibility -->
  <rect x="50" y="455" width="230" height="65" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="165" y="480" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Reports assessed for eligibility</text>
  <text x="165" y="497" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${assessed})</text>
  <!-- Arrow down -->
  <line x1="165" y1="520" x2="165" y2="605" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>
  <!-- Arrow right to excluded full text -->
  <line x1="280" y1="590" x2="400" y2="590" stroke="#000" stroke-width="1.2" marker-end="url(#arr)"/>

  <!-- Reports excluded full text -->
  <rect x="400" y="565" width="230" height="65" rx="3" fill="white" stroke="#000" stroke-width="1.2"/>
  <text x="515" y="585" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Reports excluded:</text>
  <text x="515" y="600" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Not accessible / not eligible</text>
  <text x="515" y="616" text-anchor="middle" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#000">(n = ${excFull})</text>

  <!-- Studies included -->
  <rect x="50" y="605" width="230" height="75" rx="3" fill="white" stroke="#000" stroke-width="1.5"/>
  <text x="165" y="630" text-anchor="middle" font-family="Arial,sans-serif" font-size="10" fill="#000">Studies included in review</text>
  <text x="165" y="648" text-anchor="middle" font-family="Arial,sans-serif" font-size="14" font-weight="bold" fill="#000">(n = ${included})</text>
  <text x="165" y="668" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#000">Reports of included studies (n = ${included})</text>
</svg>`;
  }
  ...
  function downloadAs(format) {
    ...
    img.onload = () => {
      ...
      canvas.width = 680; canvas.height = 820;
      const ctx = canvas.getContext("2d");
      ctx.fillStyle = "white"; ctx.fillRect(0, 0, 680, 820);
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: 7, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 10, color: "var(--muted)", alignSelf: "center", marginLeft: 4 }}>
            Download: hitam putih, background putih/transparan
          </span>
        </div>

        {/* Visual display (colored for screen) */}
        <div style={{ overflowX: "auto" }}>
          <svg ref={svgRef} viewBox="0 0 680 720" style={{ width: "100%", maxWidth: 680, display: "block", margin: "0 auto", background: "var(--bg3)", borderRadius: 8 }} xmlns="http://www.w3.org/2000/svg">
            <defs>
            ...
            {/* Phase banners */}
            {[["IDENTIFICATION","#4f9cf9",40],["SCREENING","#a78bfa",265],["ELIGIBILITY","#fbbf24",430],["INCLUDED","#34d399",610]].map(([lbl,c,y])=>(
              <g key={lbl}>
                <rect x="8" y={y} width="22" height={lbl==="INCLUDED"?80:lbl==="ELIGIBILITY"?170:lbl==="SCREENING"?155:215} rx="3" fill={c+"22"} stroke={c} strokeWidth="1"/>
                <text transform={`rotate(-90,19,${y+(lbl==="INCLUDED"?40:lbl==="ELIGIBILITY"?85:lbl==="SCREENING"?77:107)})`} x="19" y={y+(lbl==="INCLUDED"?40:lbl==="ELIGIBILITY"?85:lbl==="SCREENING"?77:107)+4} textAnchor="middle" fill={c} fontSize="8" fontWeight="700" fontFamily="monospace">{lbl}</text>
              </g>
            ))}

            {/* Row 0 header */}
            <rect x="38" y="8" width="632" height="25" rx="3" fill="#ffffff08" stroke="#2a3347" strokeWidth="1"/>
            <text x="354" y="24" textAnchor="middle" fill="#94a3b8" fontSize="9" fontWeight="700" fontFamily="monospace">IDENTIFICATION OF STUDIES VIA DATABASES AND REGISTERS</text>

            {/* ─ Row 1: 2 source boxes ─ */}
            <PBW x={38} y={40} w={225} h={80} label="Records dari Database" n={n(totalRaw)} sub="OpenAlex · Crossref · Scopus"/>
            <PBW x={425} y={40} w={245} h={80} label="Records dari Sumber Lain" n={otherRaw} sub="Grey literature · manual"/>

            {/* ─ Dedup box right ─ */}
            <PBW x={425} y={140} w={245} h={110} label="Records dihapus sebelum skrining:" n={null} sub={`Duplikat: ${n(duplicates)} | Auto-filter: ${Math.round(totalRaw*0.02)} | Lainnya: ${Math.round(totalRaw*0.01)}`} isExcl/>

            {/* Arrows row1→dedup */}
            <line x1="151" y1="120" x2="151" y2="268" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <line x1="547" y1="120" x2="547" y2="140" stroke="#64748b" strokeWidth="1"/>
            <line x1="151" y1="138" x2="425" y2="185" stroke="#64748b" strokeWidth="1" strokeDasharray="3"/>
            {/* horizontal arrow to excluded */}
            <line x1="263" y1="195" x2="425" y2="195" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>

            {/* ─ Screened box ─ */}
            <PBW x={38} y={268} w={225} h={60} label="Records diskrining" n={n(afterDupl)} sub="berdasarkan judul & abstrak"/>
            <line x1="151" y1="328" x2="151" y2="395" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <line x1="263" y1="345" x2="425" y2="345" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <PBW x={425} y={330} w={245} h={55} label="Records dieksklusi" n={n(rejected2)} sub="tidak relevan / kriteria" isExcl/>

            {/* ─ Reports sought ─ */}
            <PBW x={38} y={395} w={225} h={60} label="Reports sought for retrieval" n={n(afterScreen)} sub="artikel lolos skrining"/>
            <line x1="151" y1="455" x2="151" y2="522" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <line x1="263" y1="480" x2="425" y2="480" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <PBW x={425} y={462} w={245} h={55} label="Reports not retrieved" n={0} sub="semua berhasil diakses"/>

            {/* ─ Assessed eligibility ─ */}
            <PBW x={38} y={522} w={225} h={60} label="Reports assessed for eligibility" n={n(afterScreen)} sub="penilaian kelayakan full-text"/>
            <line x1="151" y1="582" x2="151" y2="630" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <line x1="263" y1="560" x2="425" y2="560" stroke="#64748b" strokeWidth="1" markerEnd="url(#parr)"/>
            <PBW x={425} y={545} w={245} h={60} label="Reports excluded" n={Math.max(0,n(afterScreen)-n(uploadedCount))} sub="tidak bisa diakses / tidak relevan" isExcl/>

            {/* ─ Included ─ */}
            <PBW x={38} y={630} w={225} h={72} label="Studies included in review" n={n(uploadedCount)} sub={`Reports of included studies: ${n(uploadedCount)}`} isFinal/>
          </svg>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="tw">
          <table>
            ...
            <tbody>
              {[
                ["Identifikasi","Records dari database",n(totalRaw)],
                ["Identifikasi","Records dari sumber lain",otherRaw],
                ["Pra-Skrining","Duplikat dihapus",n(duplicates)],
                ["Pra-Skrining","Records difilter otomatis",Math.round(totalRaw*0.02)],
                ["Skrining","Records diskrining (judul/abstrak)",n(afterDupl)],
                ["Skrining","Records dieksklusi",n(rejected2)],
                ["Skrining","Reports sought for retrieval",n(afterScreen)],
                ["Skrining","Reports not retrieved",0],
                ["Kelayakan","Reports assessed for eligibility",n(afterScreen)],
                ["Kelayakan","Reports excluded (full-text)",Math.max(0,n(afterScreen)-n(uploadedCount))],
                ["Inklusi","Studies included in review",n(uploadedCount)],
              ].map(([f,t,v]) => (
                <tr key={t}>
...
// Prisma Box component for colored screen display
function PBW({ x, y, w, h, label, n, sub, isExcl, isFinal }) {
  const bc = isExcl ? "#f8717133" : isFinal ? "#34d39922" : "#ffffff0a";
  const sc = isExcl ? "#f87171" : isFinal ? "#34d399" : "#2a3347";
  return (
    <g>
      <rect x={x} y={y} width={w} height={h} rx="5" fill={bc} stroke={sc} strokeWidth={isFinal?2:1.2}/>
      <text x={x+w/2} y={y+16} textAnchor="middle" fill="#94a3b8" fontSize="8.5" fontWeight="600" fontFamily="monospace">{label}</text>
      {n !== null && <text x={x+w/2} y={y+h/2+6} textAnchor="middle" fill={isFinal?"#34d399":isExcl?"#f87171":"#e2e8f0"} fontSize={isFinal?"22":"18"} fontWeight="800" fontFamily="Syne,sans-serif">{n}</text>}
      {sub && <text x={x+w/2} y={y+h-10} textAnchor="middle" fill="#475569" fontSize="7.5" fontFamily="monospace">{sub}</text>}
    </g>
  );
}
...
function downloadWord(content, filename) {
  // Build a Word-compatible HTML document with black/white tables
  const htmlContent = `
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<style>
  body { font-family: "Times New Roman",serif; font-size: 12pt; line-height: 1.8; margin: 2cm; color: #000; }
  h1 { font-size: 14pt; font-weight: bold; text-align: center; }
  h2 { font-size: 12pt; font-weight: bold; text-transform: uppercase; border-bottom: 1px solid #000; padding-bottom: 3px; margin-top: 18pt; }
  p { margin-bottom: 8pt; text-align: justify; }
  table { width: 100%; border-collapse: collapse; margin: 12pt 0; font-size: 10pt; }
  th { background: #eee; border: 1px solid #000; padding: 5pt 8pt; font-weight: bold; text-align: left; }
  td { border: 1px solid #000; padding: 5pt 8pt; vertical-align: top; }
  .caption { font-size: 10pt; font-style: italic; text-align: center; margin: 4pt 0 10pt; }
  .authors { text-align: center; font-size: 11pt; margin: 6pt 0; }
  .affil { text-align: center; font-size: 10pt; color: #333; }
</style>
</head>
<body>
${content}
...
function renderNarasiContent(text) {
  if (!text) return null;
  ...
  while (i < lines.length) {
    const line = lines[i];
    // Detect markdown table block (starts with |)
    if (line.trim().startsWith("|")) {
      ...
      out.push(<NarasiTable key={i} lines={tableLines} />);
      continue;
    }
    // Detect figure/gambar caption [GAMBAR N: ...]
    const figMatch = line.match(/\*?\*?\[?(GAMBAR|Gambar|TABEL|Tabel)\s*(\d+)[:\s]+(.*?)\]?\*?\*?/);
    if (figMatch) {
      const isFig = figMatch[1].toUpperCase() === "GAMBAR";
      out.push(
        <div key={i} style={{
          margin:"14px 0", padding:"11px 14px",
          background: isFig ? "rgba(167,139,250,.06)" : "rgba(79,156,249,.06)",
          border: `1px solid ${isFig ? "rgba(167,139,250,.25)" : "rgba(79,156,249,.25)"}`,
          borderRadius:7, display:"flex", alignItems:"center", gap:10
        }}>
          <span style={{fontSize:20}}>{isFig ? "🖼️" : "📊"}</span>
          <div>
            <div style={{fontSize:10,fontWeight:700,color: isFig ? "var(--accent2)" : "var(--accent)",textTransform:"uppercase",letterSpacing:.5,marginBottom:2}}>
              {figMatch[1]} {figMatch[2]}
            </div>
            <div style={{fontSize:11,fontFamily:"var(--fs)",color:"var(--muted)",fontStyle:"italic"}}>{figMatch[3]}</div>
          </div>
        </div>
      );
      i++; continue;
    }
    // Bold heading lines (## or **text**)
    if (line.startsWith("## ") || line.startsWith("### ")) {
      const txt = line.replace(/^#{2,3}\s*/,"");
      out.push(<div key={i} style={{fontFamily:"var(--fh)",fontWeight:700,fontSize:12,color:"var(--accent)",textTransform:"uppercase",letterSpacing:.5,margin:"16px 0 6px",paddingBottom:3,borderBottom:"1px solid var(--border)"}}>{txt}</div>);
      i++; continue;
    }
    // Bold inline **text** in paragraph
    if (line.trim()) {
      ...
      out.push(<p key={i} style={{marginBottom:8,fontFamily:"var(--fs)",fontSize:13,lineHeight:1.95}}>{parts}</p>);
    } else {
      out.push(<br key={i} />);
    }
    i++;
...
// ═══════════════════════════════════════════════
// TAB 8 — NASKAH SLR (with author panel, translate, Word)
// ═══════════════════════════════════════════════
function TabNarasi({ accepted, theme, narasiSteps, setNarasiSteps, generateNarasi, aiStatus, openStep, setOpenStep, narasiView, setNarasiView, handleImprove, settings, narasiAuthors, setNarasiAuthors }) {
