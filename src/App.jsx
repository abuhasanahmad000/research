...
- Diterbitkan dalam rentang tahun yang ditentukan`,
  ...
  async function runScreening() {
    // Use filtered articles if filters are enabled, otherwise raw
    const sourceList = searchParams.filtersEnabled ? filteredArticles : rawArticles;
    ...
    try {
      const chunks = [];
      for (let i = 0; i < Math.min(sourceList.length, 50); i += 10) {
        const batch = sourceList.slice(i, i + 10);
        setAiStatus(`skrining batch ${Math.floor(i/10)+1}/${Math.ceil(Math.min(sourceList.length,50)/10)}...`);
        const txt = await callAI(
...
Keputusan harus "accepted" atau "rejected". Berikan alasan yang jelas dan spesifik dalam Bahasa Indonesia.`,
      ...
      console.error(e);
      setArticles(sourceList.map(a => ({ ...a, status: "accepted" })));
      setScreeningDone(true);
    }
    setAiStatus(null);
    setTab("screen");
...
- Kutipan in-text minimal 5 kutipan`,
  ...
  // PRISMA numbers
  const totalRaw = filteredArticles.length;
  ...
  const renderContent = () => {
    switch (tab) {
      case "search":   return <TabSearch theme={theme} setTheme={setTheme} keywords={keywords} setKeywords={setKeywords} kwInput={kwInput} setKwInput={setKwInput} searchParams={searchParams} setSearchParams={setSearchParams} appliedFilters={appliedFilters} setAppliedFilters={setAppliedFilters} filtersDirty={filtersDirty} rawArticles={rawArticles} filteredArticles={filteredArticles} searchDone={searchDone} searchTotalCount={searchTotalCount} aiStatus={aiStatus} suggestKeywords={suggestKeywords} runSearch={runSearch} downloadCSV={downloadSearchCSV} setTab={setTab} />;
...
// ═══════════════════════════════════════════════
function TabSearch({ theme, setTheme, keywords, setKeywords, kwInput, setKwInput, searchParams, setSearchParams, appliedFilters, setAppliedFilters, filtersDirty, rawArticles, filteredArticles, searchDone, searchTotalCount, aiStatus, suggestKeywords, runSearch, downloadCSV, setTab }) {
  const toggle = (key, val) => setSearchParams(p => ({
    ...p, [key]: (p[key] || []).includes(val) ? p[key].filter(x => x !== val) : [...(p[key] || []), val]
  }));
  const setParam = (key, val) => setSearchParams(p => ({ ...p, [key]: val }));
  ...
  const showResults = searchDone && rawArticles.length > 0;
  const showFilters = showResults && searchParams.filtersEnabled;

  return (
    <div>
      {/* ── STEP 1: Theme + Keywords ── */}
      <div className="card">
        <div className="card-title">🎯 Tema Penelitian</div>
        <div className="fg">
          <label>Masukkan Tema / Topik Penelitian *</label>
          <div style={{ display: "flex", gap: 8 }}>
            <input type="text" placeholder="contoh: Cryptocurrency Stocks Correlation" value={theme} onChange={e => setTheme(e.target.value)} onKeyDown={e => e.key === "Enter" && suggestKeywords()} style={{ flex: 1 }} />
        ...
        {keywords.length > 0 && (
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 6, fontWeight: 600, letterSpacing: .5 }}>KEYWORD AKTIF (klik × untuk hapus):</div>
            <div>
              {keywords.map(k => (
      ...
      {keywords.length > 0 && (
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginBottom: 14 }}>
          <button className="btn" onClick={() => { setKeywords([]); }}>Reset Keyword</button>
          <button className="btn primary" onClick={runSearch} disabled={!keywords.length || !!aiStatus}>
            {aiStatus === "searching" ? "⏳ Mencari di OpenAlex..." : "🔍 Cari Artikel"}
          </button>
        </div>
      )}

      {/* ── STEP 3: Results + Filter Toggle ── */}
      {showResults && (
        <>
          <div className="result-counter">
            <div>
              <div className="rc-num">{filteredArticles.length}</div>
              <div className="rc-lbl">
                Artikel {searchParams.filtersEnabled ? "Setelah Filter" : "Tanpa Filter"}
                {searchTotalCount > 0 && (
                  <span style={{ display: "block", fontSize: 10, marginTop: 3 }}>
                    ({rawArticles.length} dari ~{searchTotalCount.toLocaleString()} total di OpenAlex)
                  </span>
                )}
              </div>
            </div>
            ...
            <div style={{ flex: 1, fontSize: 11, color: "var(--muted)" }}>
              Sumber: <strong>OpenAlex API</strong>
              {searchParams.filtersEnabled && (
                <div style={{ fontSize: 10, marginTop: 3 }}>
                  Filter aktif —
                  {searchParams.databases.includes("scopus") && " Scopus,"}
                  {(searchParams.qIndex || []).length > 0 && " Q: " + searchParams.qIndex.join("/") + ","}
                  {(searchParams.types || []).length > 0 && " Tipe (" + searchParams.types.length + "),"}
                  {searchParams.openAccessOnly && " OA,"}
                  {" tahun " + searchParams.yearFrom + "-" + searchParams.yearTo}
                </div>
              )}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
          ...
          <div className="card" style={{ background: searchParams.filtersEnabled ? "rgba(79,156,249,.05)" : "var(--bg2)", borderColor: searchParams.filtersEnabled ? "var(--accent)" : "var(--border)" }}>
            <label style={{ display: "flex", gap: 10, alignItems: "center", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              <input
                type="checkbox"
                checked={searchParams.filtersEnabled}
                onChange={e => setParam("filtersEnabled", e.target.checked)}
                style={{ width: 16, height: 16, cursor: "pointer" }}
              />
              <span>🎛️ Terapkan Parameter Filter</span>
              <span style={{ fontSize: 11, color: "var(--muted)", fontWeight: 400, marginLeft: "auto" }}>
                {searchParams.filtersEnabled ? "Aktif — filter di bawah akan diterapkan" : "Nonaktif — semua artikel ditampilkan"}
              </span>
            </label>
          </div>

          {/* ── STEP 4: Filter Panel (only when enabled) ── */}
          {showFilters && (
            <>
              {/* Filter group 1: Year, Type, Open Access (OpenAlex native filters) */}
              <div className="card">
                <div className="card-title">📅 Filter OpenAlex (Tahun / Tipe / Open Access)</div>
                <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 10, lineHeight: 1.5 }}>
                  Filter ini sama dengan parameter yang tersedia di website OpenAlex.
                </div>

                <div className="grid2" style={{ marginBottom: 12 }}>
                  <div className="fg">
                    <label>Tahun Mulai</label>
                    <input type="number" value={searchParams.yearFrom} onChange={e => setParam("yearFrom", +e.target.value)} />
                  </div>
                  <div className="fg">
                    <label>Tahun Akhir</label>
                    <input type="number" value={searchParams.yearTo} onChange={e => setParam("yearTo", +e.target.value)} />
                  </div>
                </div>

                <div className="fg" style={{ marginBottom: 12 }}>
                  <label>Tipe Publikasi (kosongkan = semua tipe)</label>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 4, marginTop: 6 }}>
                    {WORK_TYPES.map(t => (
                      <label key={t.id} style={{ display: "flex", gap: 6, alignItems: "center", fontSize: 11, color: "var(--text)", cursor: "pointer", padding: "3px 5px" }}>
                        <input
                          type="checkbox"
                          checked={(searchParams.types || []).includes(t.id)}
                ...
                <div className="fg">
                  ...
                  <div className="chip-group">
                    <span
                      className={`chip ${searchParams.openAccessOnly ? "on" : ""}`}
                      onClick={() => setParam("openAccessOnly", true)}
                      style={{ padding: "6px 14px" }}
                    >
                      {searchParams.openAccessOnly ? "✓ " : ""}Ya — Open Access saja
                    </span>
                    <span
                      className={`chip ${!searchParams.openAccessOnly ? "on" : ""}`}
                      onClick={() => setParam("openAccessOnly", false)}
                      style={{ padding: "6px 14px" }}
                    >
                      {!searchParams.openAccessOnly ? "✓ " : ""}Tidak — Semua artikel
                    </span>
                  </div>
                </div>
              </div>

              {/* Filter group 2: Indexing Database (Scopus) + Q-Index */}
              <div className="card">
                <div className="card-title">📚 Filter Indeks Database & Q-Index</div>
                <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 10, lineHeight: 1.5 }}>
                  Filter berbasis ISSN cross-check. Centang untuk membatasi hasil.
                </div>

                <div className="fg" style={{ marginBottom: 12 }}>
                  <label>Indeks Database</label>
                  <div className="chip-group">
                    <span
                      className={`chip ${(searchParams.databases || []).includes("scopus") ? "on" : ""}`}
                      onClick={() => toggle("databases", "scopus")}
                      style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", padding: "6px 11px", gap: 1 }}
                    >
                      <span style={{ fontWeight: 600, fontSize: 11 }}>{(searchParams.databases || []).includes("scopus") ? "✓ " : ""}Scopus</span>
                      <span style={{ fontSize: 9, color: (searchParams.databases || []).includes("scopus") ? "rgba(79,156,249,.7)" : "var(--muted)", fontStyle: "italic" }}>54.011 ISSN dari Scopus Sources Mar 2026</span>
                    </span>
                  </div>
                </div>

                <div className="fg">
                  <label>Filter Q-Index ScimagoJR</label>
                  <div className="chip-group">
                    {Q_OPTS.map(q => (
              ...
              <div className="card" style={{
                background: filtersDirty ? "rgba(251,191,36,.06)" : "var(--bg2)",
                borderColor: filtersDirty ? "var(--amber)" : "var(--border)",
                display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap"
              }}>
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: filtersDirty ? "var(--amber)" : "var(--muted)" }}>
                    {filtersDirty ? "⚠️ Perubahan Filter Belum Diterapkan" : "✓ Filter Sudah Diterapkan"}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3, lineHeight: 1.5 }}>
                    {filtersDirty
                      ? "Klik tombol Terapkan untuk merefresh angka artikel dan tabel sesuai parameter di atas."
                      : "Angka artikel dan tabel di atas sudah sesuai dengan parameter yang dipilih."}
                  </div>
                </div>
                <button
                  className="btn"
                  onClick={() => {
                    // Reset pending to match applied (cancel pending edits)
                    setSearchParams(p => ({
                      ...p,
                      yearFrom: appliedFilters.yearFrom,
                      yearTo: appliedFilters.yearTo,
                      databases: [...appliedFilters.databases],
                      qIndex: [...appliedFilters.qIndex],
                      types: [...appliedFilters.types],
                      openAccessOnly: appliedFilters.openAccessOnly,
                    }));
                  }}
                ...
                <button
                  className={`btn ${filtersDirty ? "primary" : ""}`}
                  onClick={() => {
                    // Apply: copy pending → applied snapshot
                    setAppliedFilters({
                      yearFrom: searchParams.yearFrom,
                      yearTo: searchParams.yearTo,
                      databases: [...(searchParams.databases || [])],
                      qIndex: [...(searchParams.qIndex || [])],
                      types: [...(searchParams.types || [])],
                      openAccessOnly: searchParams.openAccessOnly,
                    });
                  }}
                  disabled={!filtersDirty}
                  style={{ fontWeight: 700 }}
                >
                  {filtersDirty ? "✓ Terapkan Filter" : "✓ Sudah Diterapkan"}
                </button>
              </div>
            </>
          )}
      ...
      {/* ── Preview Table — uses filteredArticles ── */}
      {searchDone && filteredArticles.length > 0 && (
        <div className="card">
          <div className="card-title">📋 Preview Hasil ({filteredArticles.length} artikel)</div>
          <div className="tw">
            <table>
              <thead><tr>
                <th>#</th><th style={{ minWidth: 220 }}>Judul</th><th>Penulis</th><th>Tahun</th><th>Jurnal</th><th>Tipe</th><th>Q</th><th>Scopus</th><th>OA</th><th>Sitasi</th>
              </tr></thead>
              <tbody>
                {filteredArticles.slice(0, 10).map((a, i) => (
                  <tr key={a.id}>
                    ...
                    <td style={{ fontSize: 10, color: "var(--muted)" }}>{a.type}</td>
                    <td><span className={`badge ${a.q?.toLowerCase()}`}>{a.q}</span></td>
                    <td style={{ fontSize: 10 }}>{a.inScopus ? "✓" : "—"}</td>
                    <td style={{ fontSize: 10 }}>{a.isOpenAccess ? "✓" : "—"}</td>
                    <td>{a.citations}</td>
                  </tr>
                ))}
                {filteredArticles.length > 10 && (
                  <tr><td colSpan={10} style={{ textAlign: "center", color: "var(--muted)", fontSize: 10, padding: 10 }}>...dan {filteredArticles.length - 10} artikel lainnya</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {searchDone && rawArticles.length === 0 && (
        <div className="card" style={{ textAlign: "center", padding: 30, color: "var(--muted)" }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Tidak ada hasil dari OpenAlex</div>
          <div style={{ fontSize: 11 }}>Coba ganti keyword atau pakai keyword yang lebih umum.</div>
        </div>
      )}

      {/* Filter results in 0 */}
      {searchDone && rawArticles.length > 0 && filteredArticles.length === 0 && searchParams.filtersEnabled && (
        <div className="card" style={{ textAlign: "center", padding: 24, color: "var(--amber)", background: "rgba(251,191,36,.05)", borderColor: "rgba(251,191,36,.3)" }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4 }}>⚠️ Filter terlalu ketat — 0 hasil</div>
          <div style={{ fontSize: 11, color: "var(--muted)" }}>{rawArticles.length} artikel ditemukan, tapi tidak ada yang lolos filter. Coba longgarkan parameter atau matikan filter.</div>
        </div>
      )}
    </div>
  );
