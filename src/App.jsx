...
async function searchOpenAlex(keywords, yearFrom, yearTo, maxResults = 50) {
  try {
    ...
    const perPage = 50; // OpenAlex hard cap per page
    ...
    // Trim to actual maxResults requested
    ...
    return trimmed.map(w => {
      ...
      // Q-rank from Scimago lookup ONLY — no random assignment
      // If journal not in lookup, mark as "Unranked" so user knows it's not classified
...
async function searchCrossref(keywords, yearFrom, yearTo, maxResults = 50) {
  try {
    const q = keywords.join(" ");
    const rows = Math.min(maxResults, 100); // Crossref allows up to 1000 but we cap at 100
    ...
    return items.map((w, i) => {
      ...
      // Q-rank from Scimago lookup ONLY — no random
      const q_rank = SCIMAGO_Q[journal] || "Unranked";
      return {
...
export default function App() {
  ...
  const [searchParams, setSearchParams] = useState({ yearFrom: 2019, yearTo: 2025, databases: ["scopus_via","wos_via","openalex","crossref"], qIndex: [], maxResults: 100 });
  ...
  async function runSearch() {
    ...
    try {
      ...
      // User-controlled max results, default 100
      const maxR = searchParams.maxResults || 100;
      // Split between databases roughly 60/40 OpenAlex/Crossref
      ...
      const [oa, cr] = await Promise.all([
        useOpenAlex ? searchOpenAlex(keywords, searchParams.yearFrom, searchParams.yearTo, oaMax) : Promise.resolve([]),
        useCrossref ? searchCrossref(keywords, searchParams.yearFrom, searchParams.yearTo, crMax) : Promise.resolve([]),
      ]);

      let combined = deduplicateArticles([...oa, ...cr]);

      // Apply Q-index filter only if user selected specific Q levels
      if (searchParams.qIndex.length) {
        combined = filterByQ(combined, searchParams.qIndex);
      }

      // Final hard cap to user's maxResults (in case dedup didn't reduce enough)
