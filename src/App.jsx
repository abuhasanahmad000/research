...
async function searchOpenAlex(keywords, yearFrom, yearTo, maxResults = 50, searchScope = "title_abstract") {
  try {
    // Build query: OpenAlex search uses fulltext search by default
    // For title-only: use title.search filter; for title+abstract: use search parameter (broader)
    ...
    for (let page = 1; page <= pagesNeeded; page++) {
      ...
      if (searchScope === "title_only") {
        // Title-only: use filter title.search (more strict, fewer results)
        url = "https://api.openalex.org/works"
          + "?filter=title.search:" + encodeURIComponent(q)
          + ",publication_year:" + yearFrom + "-" + yearTo + ",type:article"
          + "&per-page=" + perPage
          + "&page=" + page
          + "&select=id,title,authorships,publication_year,primary_location,abstract_inverted_index,doi,concepts,cited_by_count"
          + "&mailto=slr@research.ai";
      } else {
        // Title + abstract + fulltext: use search parameter (broader, more results)
        url = "https://api.openalex.org/works"
          + "?search=" + encodeURIComponent(q)
          + "&filter=publication_year:" + yearFrom + "-" + yearTo + ",type:article"
          + "&per-page=" + perPage
          + "&page=" + page
          + "&select=id,title,authorships,publication_year,primary_location,abstract_inverted_index,doi,concepts,cited_by_count"
          + "&mailto=slr@research.ai";
    ...
    return trimmed.map(w => {
      ...
      const q_rank = SCIMAGO_Q[journal] || "Unranked";
      return {
        id: w.id,
...
async function searchCrossref(keywords, yearFrom, yearTo, maxResults = 50, searchScope = "title_abstract") {
  try {
    const q = keywords.join(" ");
    const rows = Math.min(maxResults, 100);
    let url;
    if (searchScope === "title_only") {
      // Crossref query.title for title-only search
      url = "https://api.crossref.org/works"
        + "?query.title=" + encodeURIComponent(q)
      ...
      // Standard Crossref query covers title + container + author
      url = "https://api.crossref.org/works"
    ...
    if (!res.ok) return [];
    const data = await res.json();
    const items = (data.message?.items || []).slice(0, maxResults);

    return items.map((w, i) => {
      const journal = (w["container-title"] || [])[0] || "Unknown Journal";
      const q_rank = SCIMAGO_Q[journal] || "Unranked";
      return {
        id: "cr_" + i + "_" + (w.DOI || Date.now()),
...
export default function App() {
  ...
  const [searchParams, setSearchParams] = useState({ yearFrom: 2019, yearTo: 2025, databases: ["scopus_via","wos_via","openalex","crossref"], qIndex: [], maxResults: 100, searchScope: "title_abstract" });
...
["keyword1","keyword2","keyword3"]`,
  ...
  async function runSearch() {
    ...
    // Karena user melakukan pencarian baru, semua data di tab setelahnya harus reset
    setArticles([]);
    ...
    try {
      ...
      const scope = searchParams.searchScope || "title_abstract";

      const maxR = searchParams.maxResults || 100;
      const oaMax = useOpenAlex && useCrossref ? Math.ceil(maxR * 0.6) : maxR;
      const crMax = useOpenAlex && useCrossref ? Math.ceil(maxR * 0.4) : maxR;

      const [oa, cr] = await Promise.all([
        useOpenAlex ? searchOpenAlex(keywords, searchParams.yearFrom, searchParams.yearTo, oaMax, scope) : Promise.resolve([]),
        useCrossref ? searchCrossref(keywords, searchParams.yearFrom, searchParams.yearTo, crMax, scope) : Promise.resolve([]),
      ]);

      let combined = deduplicateArticles([...oa, ...cr]);

      if (searchParams.qIndex.length) {
        combined = filterByQ(combined, searchParams.qIndex);
      }

      combined = combined.slice(0, maxR);
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="grid3" style={{ marginBottom: 12 }}>
          ...
          <div className="fg">
            <label>Maksimal Hasil</label>
            <input type="number" min="10" max="500" value={searchParams.maxResults} onChange={e => setSearchParams(p => ({ ...p, maxResults: Math.max(10, Math.min(500, +e.target.value || 100)) }))} />
          </div>
        </div>
        ...
        <div className="fg" style={{ marginBottom: 12 }}>
          <label>Lingkup Pencocokan Keyword</label>
          <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 7, lineHeight: 1.5 }}>
            Pilih di mana keyword dicari. <strong>Judul + Abstrak</strong> menghasilkan jauh lebih banyak artikel dan disarankan untuk pencarian SLR yang komprehensif.
          </div>
          <div className="chip-group">
            <span
              className={`chip ${searchParams.searchScope === "title_only" ? "on" : ""}`}
              onClick={() => setSearchParams(p => ({ ...p, searchScope: "title_only" }))}
              style={{ display:"flex", flexDirection:"column", alignItems:"flex-start", padding:"6px 11px", gap:1 }}
            >
              <span style={{ fontWeight: 600, fontSize: 11 }}>{searchParams.searchScope === "title_only" ? "✓ " : ""}Judul Saja</span>
              <span style={{ fontSize: 9, color: searchParams.searchScope === "title_only" ? "rgba(79,156,249,.7)" : "var(--muted)", fontStyle:"italic" }}>Hasil sedikit, sangat relevan</span>
            </span>
            <span
              className={`chip ${searchParams.searchScope === "title_abstract" ? "on" : ""}`}
              onClick={() => setSearchParams(p => ({ ...p, searchScope: "title_abstract" }))}
              style={{ display:"flex", flexDirection:"column", alignItems:"flex-start", padding:"6px 11px", gap:1 }}
            >
              <span style={{ fontWeight: 600, fontSize: 11 }}>{searchParams.searchScope === "title_abstract" ? "✓ " : ""}Judul + Abstrak</span>
              <span style={{ fontSize: 9, color: searchParams.searchScope === "title_abstract" ? "rgba(79,156,249,.7)" : "var(--muted)", fontStyle:"italic" }}>Hasil banyak, disarankan</span>
            </span>
          </div>
        </div>
